    ...
    func.func @main() {
    	%subop_result = subop.execution_group (){
            ...
            %vx = subop.nested_map %g_scan [@nodes::@set] (%arg0, %arg1){
                %node_stream = graph.subop.scan_node_set %arg1 : !graph.node_set<[vx_it : !graph.node_set_iterator]> @nodes::@ref({type = !graph.node_ref<[node_id : i64],[incoming : !graph.edge_set<[incoming_it : !graph.edge_set_iterator]>],[outgoing : !graph.edge_set<[outgoing_it : !graph.edge_set_iterator]>],[property : i64]>})
                tuples.return %node_stream : !tuples.tuplestream
            }
            %outgoing_sets = subop.gather %vx @nodes::@ref {outgoing => @outgoing::@set({type = !graph.edge_set<[outgoing_it : !graph.edge_set_iterator]>})}

            %ex = subop.nested_map %outgoing_sets [@outgoing::@set] (%arg0, %arg1){
                %edge_stream = graph.subop.scan_edge_set %arg1 : !graph.edge_set<[outgoing_it : !graph.edge_set_iterator]> @edges::@ref({type = !graph.edge_ref<[edge_id : i64],[from : !graph.node_ref<[node_id1 : i64],[incoming1 : !graph.edge_set<[incoming_it1 : !graph.edge_set_iterator]>],[outgoing1 : !graph.edge_set<[outgoing_it1 : !graph.edge_set_iterator]>],[property1 : i64]>],[to : !graph.node_ref<[node_id2 : i64],[incoming2 : !graph.edge_set<[incoming_it2 : !graph.edge_set_iterator]>],[outgoing2 : !graph.edge_set<[outgoing_it2 : !graph.edge_set_iterator]>],[property2 : i64]>],[edge_prop : i64]>})
